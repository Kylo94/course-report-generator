...
class TestPDFGenerator:
    ...
    def test_generate_bytes_returns_bytes(self) -> None:
        ...
        pdf_bytes = gen.generate_bytes(html)
        assert isinstance(pdf_bytes, bytes)
        assert len(pdf_bytes) > 100
        # PDF 文件头必须是 %
        assert pdf_bytes.startswith(b"%")

    def test_generate_bytes_with_chinese(self) -> None:
        """包含中文的 HTML 可以生成 PDF。"""
        gen = PDFGenerator()
        html = """
        <!DOCTYPE html>
    ...
    def test_generate_creates_file(self, tmp_path: Path) -> None:
        ...
        result = gen.generate(html, str(output))
        assert output.exists()
        assert output.stat().st_size > 100
        assert result == str(output.resolve())

    def test_generate_creates_parent_dir(self, tmp_path: Path) -> None:
        """generate 自动创建父目录。"""
        gen = PDFGenerator()
        html = "<html><body><h1>测试</h1></body></html>"
    ...
    def test_generate_with_special_chars(self) -> None:
        """HTML 含特殊字符不出错。"""
        gen = PDFGenerator()
    ...
    def test_generate_with_full_page_layout(self) -> None:
        ...
        pdf_bytes = gen.generate_bytes(html)
        assert isinstance(pdf_bytes, bytes)
        # WeasyPrint 生成的 4 页 PDF 通常 > 1KB
        assert len(pdf_bytes) > 500

    def test_generate_bytes_empty_html(self) -> None:
        """空 HTML 生成空 PDF 不出错。"""
        gen = PDFGenerator()
        pdf_bytes = gen.generate_bytes("<html></html>")
        assert isinstance(pdf_bytes, bytes)
